...
resourcename="photogallery"  # lower case, only
resourcegroup="photogallery_rg"
...
# git clone https://github.com/apilogicserver/photogallery.git
# cd photogallery
...
az group create --name photogallery_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group photogallery_rg --sku S1 --is-linux
az container create --resource-group photogallery_rg --name photogallery --image apilogicserver/photogallery:latest --dns-name-label photogallery --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//photogallery.westus.azurecontainer.io:5656'
